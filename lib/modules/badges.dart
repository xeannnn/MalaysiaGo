...
class _BadgesScreenState extends State<BadgesScreen> {
  ...
  void _checkInitialUnlocks() {
    ...
    for (var badge in allStateBadges) {
      List<String> visited = provider.visitedSites[badge.id] ?? [];
      if (badge.isComplete(visited) && !provider.celebratedBadges.contains(badge.id)) {
        provider.markBadgeCelebrated(badge.id);
  ...
  void _handleBadgeComplete(String badgeId) {
    ...
    if (!provider.celebratedBadges.contains(badgeId)) {
      provider.markBadgeCelebrated(badgeId);
...
// ============================================================
// XP PROGRESS CARD (ACCURATE, CLEAR LABEL)
// ============================================================
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Text(
                '$xpInCurrentLevel / $xpRange XP to Level ${level + 1}',
                style: TextStyle(
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      ...
      content: SizedBox(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: LevelConfig.levels.map((level) {
            return Padding(
...
class _BadgeDetailScreenState extends State<BadgeDetailScreen> {
  ...
  List<String> _visitedSites = [];

  @override
  void initState() {
    super.initState();
    final provider = Provider.of<AchievementProvider>(context, listen: false);
    _visitedSites = provider.visitedSites[widget.badge.id] ?? [];

    if (widget.claimedBonuses.contains(widget.badge.id)) {
      _bonusClaimed = true;
    }
    if (widget.badge.isComplete(_visitedSites) && !_celebrated) {
      _celebrated = true;
      WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _claimBonus() {
    ...
    if (widget.badge.isComplete(_visitedSites)) {
  ...
  @override
  Widget build(BuildContext context) {
    final unlocked = widget.badge.getUnlockedPieces(_visitedSites);
    final complete = widget.badge.isComplete(_visitedSites);
    final progress = widget.badge.getProgress(_visitedSites);
        ...
        child: Column(
          ...
          children: [
            ...
            ...widget.badge.requiredSiteIds.map((siteId) {
              final visited = _visitedSites.contains(siteId);
              return Container(
